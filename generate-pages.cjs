...
for(const l of ['en','tr']){
 ...
 const homeContactForm=composer.replace('<section class="contact-compose"','<div class="contact-compose"').replace('</section>','</div>');
 home=home.replace(/(<!-- Bottom CTA Section -->\s*)<section\b[^>]*>([\s\S]*?)<\/section>/,(_,comment,old)=>{
  ...
  return comment+'<section class="home-contact bg-stone-600 relative" aria-labelledby="home-contact-title">'+pattern+'<div class="home-contact-inner"><div class="home-contact-copy"><h2 id="home-contact-title">'+contactCopy.title+'</h2><p>'+contactCopy.intro+'</p><h3>'+contactCopy.heading+'</h3><p>'+contactCopy.body+'</p></div>'+homeContactForm+'</div></section>';
 });
