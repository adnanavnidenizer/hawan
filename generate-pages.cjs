...
for(const l of ['en','tr']){
 ...
 [4,2,1,3,5,6].forEach((asset,i)=>{
  ...
  const options=`<div class="product-options"><fieldset><legend>${tr?'Malzeme Seçimi':'Material Selection'}</legend><div class="option-list">${materials.map((label,j)=>`<label class="product-option"><input type="radio" name="material" value="${materialValues[j]}"><span><img class="wood-swatch" src="/assets/materials/${materialValues[j]}.svg?v=2" alt="" width="42" height="42" aria-hidden="true">${label}</span></label>`).join('')}</div><p class="material-note">${tr?'Renk ve doku örnekleri temsilidir; her ahşap parçası doğal farklılıklar gösterir.':'Colour and grain samples are illustrative; every piece of wood varies naturally.'}</p></fieldset><div class="size-preference"><label for="product-size">${tr?'Çap Tercihi':'Diameter Preference'}</label><div class="size-select-wrap"><select id="product-size" name="size" aria-describedby="size-note" required>${Array.from({length:8},(_,j)=>j+17).map(size=>`<option value="${size}"${size===17?' selected':""}>${size} cm</option>`).join('')}</select></div></div><p id="size-note" class="size-note">${tr?'Ürünler tam yuvarlak olmayan organik formlara sahiptir. Belirtilen çaplar yaklaşık ölçülerdir; ölçüm yönüne göre küçük farklılıklar olabilir.':'The products have organic shapes and are not perfectly round. Diameters are approximate and may vary slightly depending on the direction of measurement.'}</p></div>`;
