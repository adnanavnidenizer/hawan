...
function galleryImage(file, alt) {
 ...
 return `<img src="${a['320']}" srcset="${a['320']} 320w, ${a['640']} 640w" sizes="auto, (max-width:767px) 33vw, 25vw" width="320" height="320" data-original="${esc(original)}" data-viewer="${a.viewer}" alt="${esc(alt)}" loading="lazy" decoding="async">`;
}
...
for(const l of ['en','tr']){
 ...
 [4,2,1,3,5,6].forEach((asset,i)=>{
  ...
  const options=`<div class="product-options"><fieldset><legend>${tr?'Malzeme Seçimi':'Material Selection'}</legend><div class="option-list">${materials.map((label,j)=>`<label class="product-option"><input type="radio" name="material" value="${materialValues[j]}"><span>${label}</span></label>`).join('')}</div></fieldset><div class="size-preference"><label for="product-size">${tr?'Çap Tercihi':'Diameter Preference'}</label><div class="size-select-wrap"><select id="product-size" name="size" aria-describedby="size-note" required>${Array.from({length:8},(_,j)=>j+17).map(size=>`<option value="${size}"${size===17?' selected':""}>${size} cm</option>`).join('')}</select></div></div><p id="size-note" class="size-note">${tr?'Ürünler tam yuvarlak olmayan organik formlara sahiptir. Belirtilen çaplar yaklaşık ölçülerdir; ölçüm yönüne göre küçük farklılıklar olabilir.':'The products have organic shapes and are not perfectly round. Diameters are approximate and may vary slightly depending on the direction of measurement.'}</p></div>`;
  const detail=`<section class="shop-feature product-detail" data-product="${n}"><div class="shop-photo"><img src="/assets/models/HAWAN-0${asset}.png" alt="HAWAN ${modelNames[n-1]}" style="object-fit:contain"></div><div class="shop-copy"><span class="eyebrow">${categoryNames[0].toLocaleUpperCase(tr?'tr-TR':'en-US')}</span><h1>HAWAN ${modelNames[n-1]}</h1><p>${tr?'Üç ayaklı ahşap havan tasarımı.':'A three-legged wooden mortar design.'}</p><p class="availability">${tr?'Satış yakında açılacak.':'Available soon.'}</p><button type="button" data-product-add>${tr?'Sepete Ekle':'Add to Cart'}</button><p><a class="text-link" href="${r.shop}/">${tr?'Tüm tasarımlar':'All designs'} ↗</a></p></div></section>`;
  write(`/${tr?'tr/urun':'en/product'}/hawan-no-${n}`,shell(l,'shop','HAWAN '+modelNames[n-1],detail.replace('<p class="availability">',options+'<p class="availability">')).replace('</head>','<link rel="stylesheet" href="/css/store.css"></head>'));
 });
