...
for(const l of ['en','tr']){
 ...
 const excludedGallery=new Set(['1z_ej3UIdM4Awa6kx_7ytjLjuUO9AGCMt.png','19JJP6_u-qhbG3Rn4NIdiqwetXk0UjtHI.jpg','1SaIce1ZsJMOMUMeECBZx5js87pgc64x4.jpg','1SSxlSM9ZGpMO3KeJn4nNkg2P9ZiWEMtp.jpg']);
 const galleryFiles=[...photos,...fs.readdirSync(path.join(root,'assets/images')).sort().filter(f=>/\.(jpg|png|webp)$/i.test(f)&&!photos.includes(f))].filter(f=>!excludedGallery.has(f));
