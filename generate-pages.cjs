...
for(const l of ['en','tr']){
 ...
 [4,2,1,3,5,6].forEach((asset,i)=>{
  ...
  const options=`<div class="product-options"><fieldset><legend>${tr?'Malzeme Seçimi':'Material Selection'}</legend><div class="option-list">${materials.map((label,j)=>`<label class="product-option"><input type="radio" name="material" value="${materialValues[j]}"><span>${label}</span></label>`).join('')}</div></fieldset><div class="size-preference"><label for="product-size">${tr?'Boyut Tercihi':'Size Preference'}</label><div class="size-select-wrap"><select id="product-size" name="size" aria-describedby="size-note" required><option value="" disabled selected>${tr?'Boyut Seçin':'Select a Size'}</option>${Array.from({length:9},(_,j)=>j+17).map(size=>`<option value="${size}">${size} cm</option>`).join('')}</select></div></div><p id="size-note" class="size-note">${tr?'Ürünler tam yuvarlak olmayan organik formlara sahiptir. Belirtilen boyutlar yaklaşık ölçülerdir; ölçüm yönüne göre küçük farklılıklar olabilir.':'The products have organic shapes and are not perfectly round. Sizes are approximate and may vary slightly depending on the direction of measurement.'}</p></div>`;
